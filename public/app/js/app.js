...
var App = angular.module('singular', ['satellizer', 'ngFileUpload', 'timer', 'ngRoute', 'ngAnimate', 'ngStorage', 'ngCookies', 'pascalprecht.translate', 'ui.bootstrap', 'ui.router', 'oc.lazyLoad', 'cfp.loadingBar', 'ui.utils'])
    ...
    $rootScope.app = {
      name: 'Fighto!',
      description: 'CardBoard Development',
    ...
    $rootScope.user = {
      name:     'MK',
      job:      'Developer',
      picture:  'app/img/user/avatar_cookie.jpg'
    };
      
    //$rootScope.url = 'http://localhost:7777/pairings_server/public';
    $rootScope.url = 'http://'+window.location.host+'/fighto';
...
App.config(['$authProvider', '$stateProvider','$urlRouterProvider', '$controllerProvider', '$compileProvider', '$filterProvider', '$provide', '$ocLazyLoadProvider', 'appDependencies',
    function ($authProvider, $stateProvider, $urlRouterProvider, $controllerProvider, $compileProvider, $filterProvider, $provide, $ocLazyLoadProvider, appDependencies) {
      ...
      $stateProvider
        ...
        .state('app.mainpage', {
            url: '/mainpage',
            templateUrl: basepath('mainpage.html'),
            controller: 'MainController'
        })
        .state('app.event_details', {
            url: '/event_details/:eventId',
            templateUrl: basepath('event_details.html'),
...
App.controller('CalendarController', ["$scope", "colors", "$http", "$timeout", "touchDrag", function ($scope, colors, $http, $timeout, touchDrag) {
  ...
  $http.get('app/server/calendar/external-calendar.json').success(function(data) {
...
App.service('sidebarMemu', ["$rootScope", "$http", function($rootScope, $http) {
  'use strict';
  var menuJson = 'app/server/sidebar/sidebar-items.json',
